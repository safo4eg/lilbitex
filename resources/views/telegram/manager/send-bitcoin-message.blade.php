@if($typeValue === \App\Enums\Order\ManagerPendingExchangeTypeEnum::TRANSACTION_CREATE_ERROR->value)
...
@elseif($typeValue === \App\Enums\Order\ManagerPendingExchangeTypeEnum::TRANSACTION_SEND_ERROR->value)
🔴 Обмен №{{$orderNumber}} - ошибка при отправки транзакции.
✅ Оплата была успешно проверена, данное действие безопасное.

▶️ Нажмите кнопку "Отправить биток" для повторной попытки.
⚠️ Если повторная отправка не происходит, обратитесь к программисту.
@elseif(\App\Enums\Order\ManagerPendingExchangeTypeEnum::CHECK_PAYMENT_AND_SEND_BITCOIN->value)
